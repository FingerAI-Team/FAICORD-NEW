...
class MilVus:
    ...
    def set_env(self):
        self.client = MilvusClient(
            uri="http://" + self.host + ":19530", port=self.port
        )
        try:
            conn = connections.get_connection("default")
            if conn is not None and conn.connected():
                self.logger.info("Milvus already connected. Skipping reconnection.")
                self.conn = conn
...
class MilvusEnvManager(MilVus):
    ...
    def create_field_schema(self, schema_name, dtype=None, dim=None, max_length=200, is_primary=False):
    ...
    def delete_partition(self, collection_name, partition_name):
        try:
            ...
            self.logger.warning(f'Error while deleting partition {partition_name} in {collection_name}: {e}')
