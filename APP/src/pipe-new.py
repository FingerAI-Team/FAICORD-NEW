...
class PostProcessPipe(BasePipeline):
    ...
    def _seg_key(self, seg, ndigits=3):
        # (start,end)을 소수 n자리 반올림해서 키로 사용 (예: 0.001s 정밀도)
        s, e = seg
        return (round(float(s), ndigits), round(float(e), ndigits))
    
    def relabel_nonoverlapped_labels_new(self, file_name, diar_result, k=5):
        ...
        for (idx, emb_array, original_labels, segments) in chunk_emb_data:
            if emb_array is None or len(emb_array) == 0:  # 빈 임베딩 방어
                relabeled_diar_result.append(diar_result[idx])   # 그대로 통과
                continue
            new_labels = self.knn_cluster.relabel_by_knn(
                emb_array, original_labels, k=k,
                exclude_labels=("UNKNOWN", "filler"),
                weighted=True, keep_original_tie=True
            )
            if len(new_labels) != len(segments):
                # 보수적으로 원본 유지 (상황에 따라 raise로 바꿔도 됨)
                print(f"[WARN] new_labels({len(new_labels)}) != segments({len(segments)}) at chunk {idx}")
                relabeled_diar_result.append(diar_result[idx])
                continue
            seg2i = {}
            for i, seg in enumerate(segments):
                key = self._seg_key(seg)
                if key in seg2i:
                    # 희박하지만 라운딩 충돌 시 마지막 값을 덮어씀(로그만)
                    print(f"[WARN] rounding collision at chunk {idx} for key={key}")
                seg2i[key] = i
            relabeled_diar = []
            for (start, end), original_label in diar_result[idx]:
                if original_label in ("filler", "UNKNOWN"):
                    relabeled_diar.append(((start, end), original_label))
                    continue
                key = self._seg_key((start, end))
                if key in seg2i:
                    i = seg2i[key]
                    new_label = new_labels[i]
                    relabeled_diar.append(((start, end), new_label))
                else:
                    # 라운딩으로 못 찾으면 원 라벨 유지(필요 시 근접 매칭 추가)
                    relabeled_diar.append(((start, end), original_label))
    ...
    def apply_labels_to_full_diar_with_tie(
        self, 
        full_diar,
        ...
        min_seg_dur_for_embed: float = 0.7,
        sim_margin: float = 0.03,
        return_confidence: bool = False
    ):
        if len(relabeled_nonoverlap_diar) != len(full_diar):
            raise ValueError("full_diar와 relabeled_nonoverlap_diar의 청크 수가 다릅니다.")
        if len(audio_per_chunk) != len(full_diar):
            raise ValueError("audio_per_chunk 길이가 full_diar와 다릅니다.")

        def overlap_len(a, b):
            s1, e1 = a; s2, e2 = b
            return max(0.0, min(e1, e2) - max(s1, s2))
        
        has_proto = bool(prototypes)
        relabeled_full = []
        for ci, chunk in enumerate(full_diar):
            base = relabeled_nonoverlap_diar[ci]
            wav = audio_per_chunk[ci]
            out_chunk = []
            for (fs, fe), f_label in chunk:
                by_label_overlap = {}
                by_label_earliest = {}
                tot_ov = 0.0   # 오버랩 구간 길이 
                for (rs, re), r_label in base:
                    ov = overlap_len((fs, fe), (rs, re))
                    if ov <= 0:   # non overlapped diar과 겹침 구간이 없을 경우 
                        continue
                    tot_ov += ov
                    by_label_overlap[r_label] = by_label_overlap.get(r_label, 0.0) + ov
                    first_ov_start = max(fs, rs)
                    prev = by_label_earliest.get(r_label, first_ov_start)
                    by_label_earliest[r_label] = min(prev, first_ov_start)
                if tot_ov == 0.0:
                    best = f_label
                    purity = 0.0
                    dur = fe - fs
                    if dur >= min_seg_dur_for_embed and has_proto:
                        try:
                            seg_emb = l2norm(get_embedding_fn(wav, fs, fe))
                            sims = [(spk, cos_sim(seg_emb, proto)) for spk, proto in prototypes.items()]
                            sims.sort(key=lambda x: x[1], reverse=True)
                            if sims:
                                if len(sims) == 1 or sims[0][1] - sims[1][1] >= sim_margin:
                                    best = sims[0][0]
                        except Exception as e:
                            # print(f"[WARN] embed fail at chunk {ci}: {e}")
                            pass
                    if return_confidence:
                        out_chunk.append(((fs, fe), best, purity))
                    else:
                        out_chunk.append(((fs, fe), best))
                    continue

                dominant_label, dominant_sum = max(by_label_overlap.items(), key=lambda kv: kv[1])
                earliest_label = min(by_label_earliest.items(), key=lambda kv: kv[1])[0]
                earliest_sum = by_label_overlap.get(earliest_label, 0.0)
                purity = dominant_sum / tot_ov
                purity = max(0.0, min(1.0, purity))

                if purity >= purity_threshold:
                    if earliest_label == dominant_label:
                        best = dominant_label
                    else:
                        best = dominant_label if (earliest_sum <= dominant_sum * min_ratio) else earliest_label
                else:
                    dur = fe - fs
                    best = f_label
                    if dur >= min_seg_dur_for_embed and has_proto:
                        try:
                            seg_emb = l2norm(get_embedding_fn(wav, fs, fe))
                            cand_labels = list(by_label_overlap.keys()) or list(prototypes.keys())
                            sims = [(spk, cos_sim(seg_emb, prototypes[spk])) for spk in cand_labels if spk in prototypes]
                            sims.sort(key=lambda x: x[1], reverse=True)
                            if sims:
                                if len(sims) == 1 or sims[0][1] - sims[1][1] >= sim_margin:
                                    best = sims[0][0]
                                else:
                                    best = dominant_label if (earliest_label == dominant_label or earliest_sum <= dominant_sum * min_ratio) else earliest_label
                        except Exception as e:
                            # print(f"[WARN] embed fail at chunk {ci}: {e}")
                            best = dominant_label if (earliest_label == dominant_label or earliest_sum <= dominant_sum * min_ratio) else earliest_label
                    else:
                ...
                if return_confidence:
                    out_chunk.append(((fs, fe), best, float(purity)))
                else:
                    out_chunk.append(((fs, fe), best))

            relabeled_full.append(out_chunk)

        return relabeled_full
